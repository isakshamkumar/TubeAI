...
export default function Markdown({ markdown, className }: MarkdownProps) {
  const convertToHtml = (markdownText: string) => {
    // This is to Convert headers with Tailwind classes
    ...
    //This is to  Convert links with Tailwind
    markdownText = markdownText.replace(
      /\[([^[]+)\]\((.*)\)/gim,
      '<a href="$2" class="text-blue-500 hover:text-blue-700">$1</a>'
    )

    //This is to  Convert blockquotes with Tailwind
    markdownText = markdownText.replace(
      /^> (.*$)/gim,
      '<blockquote class="border-l-4 border-gray-500 pl-4">$1</blockquote>'
    )

    //This is to  Convert unordered lists
    markdownText = markdownText.replace(
      /^(?:-|\+|\*)\s+(.+)$/gim,
    ...
    // This is to Convert ordered lists
    markdownText = markdownText.replace(
      /^\d+\.\s+(.+)$/gim,
    ...
    //This is to  Handle paragraphs by wrapping non-tagged text in <p>
    markdownText = markdownText.replace(
      /^(?!<(?:ul|ol|li|h1|h2|h3|h4|h5|h6|blockquote|a|strong|i|em|div|p)\b)[^<]+/gim,
      '<p class="text-[12px]">$&</p>'
    )

    //This is to  Replace line breaks with <br>
    markdownText = markdownText.replace(/\n/g, "<br>")
